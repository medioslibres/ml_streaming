...
int main( int  argc, char *argv[], char *envp[]) {
  ...
  if (!gtk_init_check (&argc, &argv)) {
    fprintf(stderr, "La pantalla no se puede inicializar, esperar a que la interfaz se active:-P\n");
    exit (-1);
  }

  if (envp[i] == NULL) {
    fprintf(stderr, "Que broma, Variables de ambiente no encontradas?\n");
    exit(-1);
  }
  

  /* Define el área compartida                                          */
  /* 00 significa el streaming detenido, no se actualizan etiqueta      */
  /* 01 significa el streaming detenido, actualizar etiqueta            */
  /* 10 significa el streaming ha comenzado, no se actualice etiqueta   */
  /* 11 significa el streaming ha comenzado, actualice etiqueta         */
  ...
  /* si no existe, crea el directorio config */
  strcat(home,"/.ml_streaming");
  if ( !(directory = opendir(home)) ) {
    if (mkdir(home, S_IRWXU ))
      fprintf(stderr, "Error: %s no se puede crear\n", home);
  }
  else
    closedir(directory);

  /* establece la ubicación de los archivos de configuración */
  strcpy (ml_streaming_cfg, home);
  strcat (ml_streaming_cfg, "/ml_streaming.cfg");
  strcpy (darkice_cfg, home);
  strcat (darkice_cfg, "/darkice.cfg");
  strcpy (ml_streaming_temp, home);
  strcat (ml_streaming_temp, "/ml_streaming.temp");

  /* el FIFO para la ventana de depuración de DarkIce */
  unlink(ml_streaming_temp);
  if(mkfifo(ml_streaming_temp, S_IRWXU ))
    fprintf(stderr, "error al crear la FIFO en %s\n", ml_streaming_temp);
  darkice_fd = open (ml_streaming_temp, O_RDONLY | O_NONBLOCK);
  ...
  bindtextdomain ("ml_streaming", locale);
  textdomain ("ml_streaming");

  /* comprobar si existe DarkIce en la ruta */
  i = 0; j = 5;
  ...
  while ( (sscanf(envp[i-1]+j,"%[^:]:",bar) != -1) && j){ 
    ...
    j = 0;   /* 0 significa que DarkIce fue encontrado */
    close (fd);
    }
  }
  ...
  g_timeout_add (1000, (GSourceFunc) status_loop, 0); /* status_loop() actualiza el estado de streaming */
  
  window = gtk_window_new (GTK_WINDOW_TOPLEVEL);
  button_start = gtk_button_new_with_label ( gettext("Iniciar Streaming") );
  button_stop = gtk_button_new_with_label ( gettext("Detener Streaming") );
  button_detail = gtk_button_new_with_label ( gettext("Mostrar Detalles") );
  show_detail = 0;
  label_status = gtk_label_new ( gettext("Status de Streaming: Detenido"));
  table = gtk_table_new (2, 1, FALSE);
  vbox = gtk_vbox_new (FALSE, 0); /* paquetes de menu_bar y de tabla */


  /* Ventana principal  */
  gtk_window_set_title (GTK_WINDOW (window), "ml_streaming");
  gtk_widget_set_size_request (GTK_WIDGET (window), SIZE_X, SIZE_Y); 
  gtk_container_set_border_width (GTK_CONTAINER (window), 0);


  /* establece las cosas del menú superior */
  menu_bar = gtk_menu_bar_new();  

  menu_file = gtk_menu_new();
  menu_item_file = gtk_menu_item_new_with_label ( gettext("Archivo") );
  menu_item_open = gtk_menu_item_new_with_label ( gettext("Abrir Configuración") );
  menu_item_save = gtk_menu_item_new_with_label ( gettext("Grabar Configuración") );
  menu_item_quit_nd = gtk_menu_item_new_with_label ( gettext("Salir y no detener DarkIce") );
  menu_item_quit = gtk_menu_item_new_with_label ( gettext("Salir") );
  ...
  menu_item_help = gtk_menu_item_new_with_label ( gettext("Ayuda") );
  menu_item_about = gtk_menu_item_new_with_label ( gettext("Acerca de") );
  ...
  /* establece el diálogo acerca de... */
  dialog_about = gtk_about_dialog_new ();
  const gchar *authors[] = {"Jaime Ernesto Mora", NULL};
  gtk_about_dialog_set_program_name( GTK_ABOUT_DIALOG( dialog_about), "ml_streaming");
...
/*  gtk_about_dialog_set_license_type( GTK_ABOUT_DIALOG(dialog_about), GTK_LICENSE_GPL_3_0);*/
  gtk_about_dialog_set_website( GTK_ABOUT_DIALOG(dialog_about), "http://ml_streaming.medioslibres.org.ve");

  /* establece la ventana de salida */
  dialog_darkkill = gtk_dialog_new ();
  label_darkkill = gtk_label_new (gettext("DarkIce se está ejecutando.\n ¿Quieres matar el proceso DarkIce?\n"));
  button_yes_darkkill = gtk_button_new_with_label (gettext("Si"));
  button_no_darkkill = gtk_button_new_with_label (gettext("No"));

  gtk_window_set_title(GTK_WINDOW (dialog_darkkill), gettext("Darkice se está ejecutando"));
  ...
  /* establece los selectores de archivos */
  file_open = gtk_file_chooser_dialog_new ( gettext("Abrir Archivo"),
                                          NULL,
                                          GTK_FILE_CHOOSER_ACTION_OPEN,
                                          GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
                                          GTK_STOCK_OPEN, GTK_RESPONSE_ACCEPT,
                                          NULL);
  file_save = gtk_file_chooser_dialog_new ( gettext("Grabar Archivo"),
                                          NULL,
                                          GTK_FILE_CHOOSER_ACTION_SAVE,
                                          GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
                                          GTK_STOCK_OPEN, GTK_RESPONSE_ACCEPT,
                                          NULL);
  file_localdump = gtk_file_chooser_dialog_new ( gettext("Archivo de volcado Local"),
                                          NULL,
                                          GTK_FILE_CHOOSER_ACTION_OPEN,
                                          GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
                                          GTK_STOCK_OPEN, GTK_RESPONSE_ACCEPT,
  ...
  /* Establece los elementos del cuadro de configuración*/
  ...
  label_notebook_server = gtk_label_new ( gettext("Opciones de Servidor"));
  label_notebook_audio = gtk_label_new ( gettext("Opciones de Audio"));
  label_notebook_description = gtk_label_new ( gettext("Descripción del Streaming"));
  ...
  /* Opciones de Servidor Widgets */
  label_icecast = gtk_label_new ( gettext("Destino del Streaming: "));
  ...
  label_server = gtk_label_new ( gettext("Servidor: "));
  entry_server = gtk_entry_new ();
  label_port = gtk_label_new ( gettext("Puerto: "));
  entry_port = gtk_entry_new ();
  label_mountpoint = gtk_label_new ( gettext("Punto de Montaje: "));
  entry_mountpoint = gtk_entry_new ();
  label_pass = gtk_label_new ( gettext("Contraseña: "));
  entry_pass = gtk_entry_new ();
  gtk_entry_set_visibility (GTK_ENTRY(entry_pass), FALSE);
  
  label_verbosity = gtk_label_new ( gettext ("Nivel de detalle: "));
  ...
  label_remotedump = gtk_label_new ( gettext ("Archivo de volcado remoto: "));
  entry_remotedump = gtk_entry_new ();
  
  button_localdump = gtk_button_new_with_label ( gettext("Archivo de volcado local"));
  entry_localdump = gtk_entry_new ();

  checkbutton_adddate = gtk_check_button_new_with_label ( gettext ("Agregar fecha a nombre del archivo de volcado local? "));

  /* Opciones del Servidor Final Widgets */

  /* Opciones de Audio Widgets */
  label_format = gtk_label_new ( gettext("Formato: "));
  ...
  label_bitratemode = gtk_label_new ( gettext("Modo Bit Rate: "));
  combo_bitratemode = gtk_combo_box_text_new_with_entry ();
  gtk_combo_box_text_append_text ( GTK_COMBO_BOX_TEXT (combo_bitratemode), gettext("Constante"));
  gtk_combo_box_text_append_text ( GTK_COMBO_BOX_TEXT (combo_bitratemode), gettext("Pomedio"));
  gtk_combo_box_text_append_text ( GTK_COMBO_BOX_TEXT (combo_bitratemode), gettext("Variable"));
  gtk_entry_set_text (GTK_ENTRY (gtk_bin_get_child (GTK_BIN (combo_bitratemode))), gettext("Constante"));

  label_bitrate = gtk_label_new ( gettext("Bit Rate: "));
  ...
  label_samplerate = gtk_label_new ( gettext("Frecuencia: "));
  ...
  label_quality = gtk_label_new ( gettext("Calidad: "));
  ...
  label_bitspersample = gtk_label_new ( gettext("Bits por muestra: "));
  ...
  label_channel = gtk_label_new ( gettext("Número de canales: "));
  ...
  label_buffersize = gtk_label_new ( gettext ("Tamaño de Buffer: "));
  ...
  label_device = gtk_label_new ( gettext ("Dispositivo de entrada: "));
  ...
  /* Opciones de Audio finales Widgets */
  
  /* widgets de configuración - etiqueta + entrada */
  label_radioname = gtk_label_new ( gettext("Nombre de la Radio: "));
  entry_radioname = gtk_entry_new ();
  label_url = gtk_label_new ( gettext("URL: "));
  entry_url = gtk_entry_new ();
  label_description = gtk_label_new ( gettext("Descripción: "));
  entry_description = gtk_entry_new ();
  label_genre = gtk_label_new ( gettext("Género: "));
  entry_genre = gtk_entry_new ();
  
  /* widgets de configuración - label + combo box */

  
  label_public = gtk_label_new ( gettext("Public: "));
  combo_public = gtk_combo_box_text_new_with_entry ();
  gtk_combo_box_text_append_text ( GTK_COMBO_BOX_TEXT (combo_public), "si");
  ...
  /* elementos vista de texto */
  tag = gtk_text_tag_new ("");
  tag_table = gtk_text_tag_table_new();
  gtk_text_tag_table_add(tag_table, tag);
  buffer = gtk_text_buffer_new(tag_table);
  foo = gettext("ml_streaming - Herramienta gráfica para DarkIce - Revolucionando la Patria Grande");
  gtk_text_buffer_set_text(buffer, foo,strlen(foo));
  text = gtk_text_view_new_with_buffer (buffer);
  gtk_text_view_set_editable(GTK_TEXT_VIEW (text), FALSE);    

  /* desplazarse por el control textview */
  ...
  /* Desde la Tierra Mágica de los Shamatari :-) */
  gtk_box_pack_end (GTK_BOX (vbox), table, TRUE, TRUE, 5);
  gtk_container_add (GTK_CONTAINER (window), vbox);


  /* setear los widgetsde los tooltips */
  set_all_tooltips();
  ...
  /* Empacar el área de notas, los botones y los textview dentro de la tabla principal */
  ...
  /* pestaña principal del área de notas */
