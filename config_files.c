...
gboolean ml_streaming_config(char opt, char *save_path) {
  int i;
  char mp[128];
  char *server;
  char *port;
  ...
  if (!( f_ml_streaming_cfg = (opt != 0)? fopen(ml_streaming_cfg, "w") : fopen(save_path, "w"))) {
      printf("Error: Cannot open %s\n", (opt != 0)? ml_streaming_cfg: save_path);
      return FALSE;
  }
  ...
  /* Entry writes */
  fprintf (f_ml_streaming_cfg, "server=%s\n", server);
  fprintf (f_ml_streaming_cfg, "port=%s\n", port);
  fprintf (f_ml_streaming_cfg, "mountpoint=%s\n", mp);
  fprintf (f_ml_streaming_cfg, "pass=%s\n", pass);
  fprintf (f_ml_streaming_cfg, "radioname=%s\n", radioname);
  fprintf (f_ml_streaming_cfg, "description=%s\n", description);
  fprintf (f_ml_streaming_cfg, "url=%s\n", url);
  fprintf (f_ml_streaming_cfg, "genre=%s\n", genre);
  fprintf (f_ml_streaming_cfg, "icecast=%s\n", icecast);
  fprintf (f_ml_streaming_cfg, "format=%s\n", format);
  fprintf (f_ml_streaming_cfg, "bitrate=%s\n", bitrate);
  fprintf (f_ml_streaming_cfg, "bitratemode=%s\n", bitratemode);
  fprintf (f_ml_streaming_cfg, "samplerate=%s\n", samplerate);
  fprintf (f_ml_streaming_cfg, "quality=%s\n", quality);
  fprintf (f_ml_streaming_cfg, "bitspersample=%s\n", bitspersample);
  fprintf (f_ml_streaming_cfg, "channel=%s\n", channel);
  fprintf (f_ml_streaming_cfg, "buffersize=%s\n", buffersize);
  fprintf (f_ml_streaming_cfg, "device=%s\n", device);
  fprintf (f_ml_streaming_cfg, "public=%s\n", public);
  fprintf (f_ml_streaming_cfg, "verbosity=%s\n", verbosity);
  fprintf (f_ml_streaming_cfg, "remotedump=%s\n", remotedump);
  fprintf (f_ml_streaming_cfg, "localdump=%s\n", localdump);
  fprintf (f_ml_streaming_cfg, "adddate=%c\n", adddate);
...
gboolean darksnow2darkice_cfg() {
  ...
  if (!(f_ml_streaming_cfg = fopen(ml_streaming_cfg, "r"))) {
    printf("Error: Cannot open %s\n", ml_streaming_cfg);
    exit (-1);
  }
